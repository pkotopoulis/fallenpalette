...
import { ALL_PAINTS, paintId, paintPath } from "./data/paints";
import { confusablePairs } from "./utils/cvd";
...
describe("colour vision", () => {
  const own = (...ids: string[]) =>
    localStorage.setItem("paintxref_collection", JSON.stringify(ids));
  const setCvd = (type: string, sim = false) => {
    localStorage.setItem("fp_cvd", type);
    localStorage.setItem("fp_cvd_sim", sim ? "1" : "0");
  };

  /**
   * A real catalog pair a deuteranope cannot separate, close enough normally to
   * also surface as a substitute (findSubstitutes cuts off at 14).
   *
   * Derived from the data rather than hardcoded so that recolouring a paint
   * cannot quietly turn these assertions vacuous.
   */
  const PAIR = confusablePairs(ALL_PAINTS, "deuteranopia", Infinity)
    .find(p => p.normal <= 13)!;

  it("has a usable fixture pair to work from", () => {
    expect(PAIR, "no confusable pair under 13 in the catalog").toBeTruthy();
    expect(PAIR.a.hex).not.toBe(PAIR.b.hex);
  });

  it("offers the deficiencies in the navbar, off by default", () => {
    at("/");
    const select = document.querySelector<HTMLSelectElement>(".cvd-select");
    expect(select).toBeTruthy();
    expect(select!.value).toBe("none");
    expect([...select!.options].map(o => o.value))
      .toEqual(["none", "protanopia", "deuteranopia", "tritanopia"]);
  });

  it("only offers the simulation once there is a deficiency to simulate", () => {
    at("/");
    expect(document.querySelector(".cvd-sim-btn")).toBeNull();
    fireEvent.change(document.querySelector(".cvd-select")!, { target: { value: "deuteranopia" } });
    expect(document.querySelector(".cvd-sim-btn")).toBeTruthy();
  });

  it("remembers the choice, but keeps it out of the URL", () => {
    // It describes the person looking, not the view, so a shared link must not
    // carry it. Persistence is the whole point — nobody wants to re-pick this.
    at("/colours");
    fireEvent.change(document.querySelector(".cvd-select")!, { target: { value: "protanopia" } });
    expect(localStorage.getItem("fp_cvd")).toBe("protanopia");
    expect(window.location.search).not.toContain("protanopia");
    cleanup();

    at("/colours");
    expect(document.querySelector<HTMLSelectElement>(".cvd-select")!.value).toBe("protanopia");
  });

  it("leaves swatches alone until the simulation is switched on", () => {
    // The distinction the feature rests on: choosing a deficiency asks for the
    // warnings, not for repainted swatches.
    const hex = PAIR.a.hex.toLowerCase();
    const swatchHexes = () =>
      [...document.querySelectorAll<HTMLElement>(".swatch")]
        .map(s => s.style.background.toLowerCase());

    setCvd("deuteranopia", false);
    at(paintPath(PAIR.a));
    expect(swatchHexes(), "unsimulated swatch should show the real colour").toContain(hex);
    cleanup();

    setCvd("deuteranopia", true);
    at(paintPath(PAIR.a));
    expect(swatchHexes(), "simulated swatch should not show the real colour").not.toContain(hex);
  });

  it("badges an owned substitute the viewer cannot tell from the target", () => {
    // The inverted insight: the app rates this pair merely Close or Approx, but
    // for this viewer it is a perfect stand-in.
    own(paintId(PAIR.b));
    setCvd("deuteranopia");
    at(paintPath(PAIR.a));
    ...
    const block = document.querySelector(".sub-block");
    expect(block, "expected a From your paints block").toBeTruthy();
    expect(block!.textContent).toContain(PAIR.b.name);
    expect(block!.querySelector(".cvd-same-badge"), "expected a Same to you badge").toBeTruthy();
  });

  it("does not badge that same substitute for a normal-sighted viewer", () => {
    own(paintId(PAIR.b));
    at(paintPath(PAIR.a));
    expect(document.querySelector(".sub-block")!.textContent).toContain(PAIR.b.name);
    expect(document.querySelector(".cvd-same-badge")).toBeNull();
  });

  it("lists confusable pots on the shelf", () => {
    own(paintId(PAIR.a), paintId(PAIR.b));
    setCvd("deuteranopia");
    at("/my-paints");

    const pair = document.querySelector(".cvd-pair");
    expect(pair, "expected a confusable pair card").toBeTruthy();
    expect(pair!.textContent).toContain(PAIR.a.name);
    expect(pair!.textContent).toContain(PAIR.b.name);
    // Both distances are shown, so the claim is auditable rather than asserted.
    expect(pair!.querySelector(".cvd-pair-foot")!.textContent).toContain(PAIR.normal.toFixed(1));
    expect(pair!.querySelector(".cvd-pair-foot")!.textContent).toContain(PAIR.seen.toFixed(1));
  });

  it("says so plainly when the shelf holds no confusable pair", () => {
    own(paintId(PAIR.a));
    setCvd("deuteranopia");
    at("/my-paints");
    const block = document.querySelector(".cvd-block");
    expect(block).toBeTruthy();
    expect(block!.querySelector(".sub-empty")).toBeTruthy();
    expect(block!.querySelector(".cvd-pair")).toBeNull();
  });

  it("hides the shelf warning entirely with no deficiency selected", () => {
    own(paintId(PAIR.a), paintId(PAIR.b));
    at("/my-paints");
    expect(document.querySelector(".cvd-block")).toBeNull();
  });
});
